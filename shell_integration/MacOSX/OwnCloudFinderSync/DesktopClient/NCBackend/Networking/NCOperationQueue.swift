        ...
        if isCancelled {
            ...
            NCNetworking.shared.deleteMetadata(metadata, onlyLocalCache: onlyLocalCache) { errorCode, errorDescription in
                if errorCode != 0 {
                    ...
                    print("Error with delete operation")
                }
                self.finish()
            }
        }
        ...
        if isCancelled {
            ...
            if move {
                NCNetworking.shared.moveMetadata(metadata, serverUrlTo: serverUrlTo, overwrite: overwrite) { errorCode, errorDescription in
                    if errorCode != 0 {
                        // NCContentPresenter.shared.messageNotification("_error_", description: errorDescription, delay: NCGlobal.shared.dismissAfterSecond, type: NCContentPresenter.messageType.error, errorCode: errorCode)
                        print("Error with copy/move operation")
                    }
                    self.finish()
                }
            } else {
    ...
    override func start() {
        if isCancelled {
            ...
            if metadata.directory {
                ...
                NCCommunication.shared.readFileOrFolder(serverUrlFileName: serverUrl, depth: "0", showHiddenFiles: true) { account, files, _, errorCode, _ in
                    ...
                    if (errorCode == 0) && (directory?.etag != files.first?.etag || self.selector == NCGlobal.shared.selectorDownloadAllFile) {
                        ...
                        NCCommunication.shared.readFileOrFolder(serverUrlFileName: serverUrl, depth: "1", showHiddenFiles: true, queue: NCCommunicationCommon.shared.backgroundQueue) { account, files, _, errorCode, _ in
